...
const Discover = () => { 
    ...
    if(tab === "expired") { 
        ...
        return (
            <Filter PageTitle={"Dropped Domains"} First={100} Skip={0} Tab={tab} OrderBy={"expires"} OrderDirection={"desc"} Filter={filterStr} />
        )
        ...
        const filterStr = getFilterObjStr(filter);
        return ( 
            <Filter PageTitle={"Expiring Soon"} First={100} Skip={0} Tab={tab} OrderBy={"expires"} OrderDirection={"asc"} Filter={filterStr} />
        )
    } else if(tab === "premium") { 
        ...
        const filterStr = getFilterObjStr(filter);
        return ( 
            <Filter PageTitle={"Premium Right Now"} First={100} Skip={0} Tab={tab} OrderBy={"expires"} OrderDirection={"asc"} Filter={filterStr} />
        )
    } else if(tab === "registered") {
        filter.registered_not = null;
        const filterStr = getFilterObjStr(filter);
        return (
            <Filter PageTitle={"Recently Registered"} First={100} Skip={0} Tab={tab} OrderBy={"registered"} OrderDirection={"desc"} Filter={filterStr} />
        )
    } else {
        const filterStr = getFilterObjStr(filter);
        return (
            <Filter PageTitle={"Browse ENS Domains"} First={100} Skip={0} Tab={tab} OrderBy={"created"} OrderDirection={"desc"} Filter={filterStr} />
        )
    } 
};
